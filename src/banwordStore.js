...
// 채팅 메시지마다(=아주 자주) 검사하는 목록이라, 매번 word.toLowerCase()를 반복하지 않도록
// 불러오거나 추가할 때 한 번만 소문자로 미리 변환해서 같이 들고 있음 (_wordLower).
...
function save() {
  // _wordLower는 메모리 안에서만 쓰는 캐시라서 파일에는 안 남김 (word만 저장해도 다음 로드 때 다시 계산됨)
  fs.writeFileSync(config.banwordsFilePath, JSON.stringify(words.map(stripInternal), null, 2), "utf8");
}

// 바깥(대시보드 API 등)으로 내보낼 때는 내부 캐시 필드(_wordLower)를 감추고 word/action/enabled만 보여줌
